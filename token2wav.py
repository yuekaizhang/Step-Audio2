...
class Token2wav(torch.nn.Module):
    def __init__(self, model_dir: str, enable_trt: bool = False, device_id: int = 0, streaming: bool = False, dtype: torch.dtype = torch.float16):
        ...
        if enable_trt:
            if streaming:
                self.load_trt(
                    f'{model_dir}/flow.decoder.estimator.{self.dtype}.static_batch.chunk.plan',
                    f'{model_dir}/flow.decoder.estimator.chunk.fp32.static_batch.onnx',
                    1,
...
if __name__ == "__main__":
    ...
    # streaming token2wav using pytorch
    # token2wav = Token2wav('Step-Audio-2-mini/token2wav')

    # streaming token2wav using tensorrt
